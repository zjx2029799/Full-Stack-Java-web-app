			...
			<thead>
				<tr>
					...
					<th style="width: 70px; ">操作</th>		
									...
									<c:forEach items="${mlist}" var="myf">
										<tr>
											...
											<td class="action-td"><p:doPower value="用户:下载">
													<a
														href="${pageContext.request.contextPath }/com/download?cid=${myf.mcid}"
														class="btn btn-small btn-warning"> <i class="icon-ok">
															下载</i>
													</a>
												</p:doPower> <p:doPower value="删除文件">
													<a
														href="${pageContext.request.contextPath }/MyFile/delMyFile?cid=${myf.mcid}"
						...
						<ul class="pager">
							<c:if test=""></c:if>
						  <li class="previous <c:if test="${page==1}">disabled</c:if>" ><a href="<c:if test="${page!=1}">${pageContext.request.contextPath }/MyFile/showMyFiles?count=${page-1}&id=${id}</c:if>#">&larr; 上一页</a></li>
						  <li class="next <c:if test="${page==total}">disabled</c:if>"><a href="<c:if test="${page!=total}">${pageContext.request.contextPath }/MyFile/showMyFiles?count=${page+1}&id=${id}</c:if>#">下一页 &rarr;</a></li>
						</ul>
          </div>
        </div>
