...
<div id="search">
<form action="${pageContext.request.contextPath}/com/select">
			...
			<thead>
				<tr>
					...
					<th style="width: 80px; ">下载量</th>	
									...
									<c:forEach items="${comlist}" var="com" varStatus="status">
										<tr>
											...
											<td class="action-td"><p:doPower value="用户:下载">
													<a
														href="${pageContext.request.contextPath }/com/download?cid=${com.cid}"
						...
						<%-- <ul class="pager">
							<c:if test=""></c:if>
						  <li class="previous <c:if test="${page==1}">disabled</c:if>" ><a href="<c:if test="${page!=1}">${pageContext.request.contextPath }/com/showFilesRank?count=${page-1}</c:if>#">&larr; 上一页</a></li>
						  <li class="next <c:if test="${page==total}">disabled</c:if>"><a href="<c:if test="${page!=total}">${pageContext.request.contextPath }/com/showFilesRank?count=${page+1}</c:if>#">下一页 &rarr;</a></li>
						</ul> --%>
          </div>
        </div>
